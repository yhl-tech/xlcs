...
**塞拉测试系统**是基于 Vue3 + Vite 的在线罗夏墨迹心理测评前端：以 **WebRTC** 承载 AI 实时语音施测，采集缩放/旋转/笔迹、问卷与音频等多维数据并上传，完成分析到报告等待的闭环；施测与后测等 **System Prompt 持续撰写与调优**，约束话术边界与中立施测风格。技术栈：Vue3、Vite、Pinia、Vue Router、Axios、WebRTC、Three.js。

### 主要功能

- **墨迹图版与交互**：10 张标准图版，支持缩放、旋转、画笔轨迹等结构化采集
- **实时语音施测**：WebRTC 低延迟对话，麦克风采集、转写、逐图重连与会话管理
- **后测问卷与上传**：多类型数据分批上传、进度与断点恢复，支撑报告生成
- **提示词工程**：施测/后测等提示词随业务迭代调优，与语音交互行为对齐
- **报告与工程体验**：报告等待与下载；Vite 分包、资源分类与懒加载优化首屏与交互负载

/**
 * 主流程脚本：负责初始化页面、绑定事件、处理会话/绘图/音频等逻辑。
 * 为便于维护，所有核心常量与状态在 appState.js 中统一导出。
 */
...
function syncBasicInfoDraftFromInputs() {
    ...
    BASIC_INFO_FIELDS.forEach(field => {
        if (inputs[field]) {
            nextDraft[field] = inputs[field].value || '';
        }
    });
...
function setupBasicInfoDraftListeners() {
    ...
    BASIC_INFO_FIELDS.forEach(field => {
        ...
        input.addEventListener('input', () => {
            state.basicInfoDraft[field] = input.value || '';
...
function fetchUserInfo(){
    const draft = syncBasicInfoDraftFromInputs();

...
;(function (window) {
  ...
  class InteractionTracker {
    ...
    trackZoom(direction) {
      ...
      if (this.status !== "active") {
        console.warn(
          "[InteractionTracker] trackZoom 状态不是 active:",
          this.status
        )
        return
      }
    ...
    trackRotate(angle) {
      ...
      if (this.status !== "active") {
        console.warn(
          "[InteractionTracker] trackRotate 状态不是 active:",
          this.status
        )
        return
      }
      ...
      if (this.config.debug) {
        console.log(
          `[InteractionTracker] 记录旋转操作: 图版${plateKey}, 角度=${angle}度`
        )
      }
    }
    ...
    trackNavigation(direction) {
      ...
      if (this.config.debug) {
        console.log(
          `[InteractionTracker] 记录导航操作: 图版${plateKey}, 方向=${direction}`
        )
      }
    }
    ...
    serialize() {
      return {
        ...
        currentTrack: this.currentTrack
          ? JSON.parse(JSON.stringify(this.currentTrack))
          : null,
    ...
    restore(snapshot = {}) {
      ...
      this.currentPlateIndex =
        typeof currentPlateIndex === "number" ? currentPlateIndex : null
      this.testStartTime =
        typeof testStartTime === "number" ? testStartTime : null
      this.timestamps =
      ...
      this.currentTrack = Array.isArray(currentTrack)
        ...
        : null
      this.currentTrackStartTime =
        typeof currentTrackStartTime === "number" ? currentTrackStartTime : null
      // 恢复图版键，如果没有则使用当前图版键作为后备（向后兼容）
      this.currentTrackPlateKey =
        typeof currentTrackPlateKey === "string"
          ? currentTrackPlateKey
          : this.currentTrack
          ? this._getCurrentPlateKey()
          : null
    ...
    getStatistics(plateIndex = null) {
      if (plateIndex !== null) {
        ...
        return {
          ...
          drawingTracks:
            this.data.drawingTracks[plateKey] === 0
              ? 0
              : Object.keys(this.data.drawingTracks[plateKey]).length,
        }
      }
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        if (
          this.data.drawingTracks[key] !== 0 &&
          typeof this.data.drawingTracks[key] === "object"
        ) {
    ...
    printAllPlatesStatistics() {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        allPlatesStats.plates[plateKey] = {
          zoom: {
            count: this.data.zoom[plateKey]
              ? this.data.zoom[plateKey].length
              : 0,
            data: this.data.zoom[plateKey] || [],
          },
          rotate: {
            count: this.data.rotate[plateKey]
              ? this.data.rotate[plateKey].length
              : 0,
            data: this.data.rotate[plateKey] || [],
          },
          navigation: {
            count: this.data.navigation[plateKey]
              ? this.data.navigation[plateKey].length
              : 0,
      ...
      if (this._eventListeners[eventName]) {
        this._eventListeners[eventName].forEach((sub) => {
          try {
            ...
            console.error(
              `[InteractionTracker] 事件回调错误 (${eventName}):`,
              error
            )
          }
        })
  ...
  window.InteractionTracker = {
    ...
    printDataStructures: (plateIndex) =>
      trackerInstance.printDataStructures(plateIndex),
    ...
    // 内部方法（供集成使用）
    _updateCurrentPlate: (plateIndex) =>
      trackerInstance.updateCurrentPlate(plateIndex),

...
export async function startOperationReactionTest(onComplete = null) {
  ...
  try {
    ...
    // 所有操作完成后，播报第二段文本
    const finalText =
      "第二项测试时我会依次给你展示 10 张图片，你只需要告诉我在图片中看到了什么，并描述你看到的东西、联想到的东西。不管看见什么，都可以直接描述，没有正确与错误。在一张图片中你可能会看到多个物体和场景，描述得越详细越好。测试过程中，你可以像刚才那样调整图片大小或者旋转观察不同的角度，并用画笔标记你看到的物体或场景。如果您确认清楚了测试的流程，那就可以点击“进入”按钮，开始正式的测试。"
